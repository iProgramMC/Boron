...
Abstract:
	...
	functions for the i386 platform.
...
{
	return Pte.PteHardware & MM_I386_PTE_PRESENT;
}

bool MmIsPoolHeaderPte(MMPTE Pte)
{
	return !MmIsPresentPte(Pte) && (Pte.PteHardware & MM_I386_DPTE_ISPOOLHDR);
}

bool MmWasPresentPte(MMPTE Pte)
{
	return !MmIsPresentPte(Pte)
		&& !(Pte.PteHardware & MM_I386_DPTE_ISPOOLHDR)
		&& (Pte.PteHardware & MM_I386_DPTE_WASPRESENT);
}
...
bool MmIsCommittedPte(MMPTE Pte)
{
	return !MmIsPresentPte(Pte)
		&& !(Pte.PteHardware & MM_I386_DPTE_ISPOOLHDR)
		&& (Pte.PteHardware & MM_I386_DPTE_DECOMMITTED);
}
...
bool MmIsDecommittedPte(MMPTE Pte)
{
	return !MmIsPresentPte(Pte)
		&& !(Pte.PteHardware & MM_I386_DPTE_ISPOOLHDR)
		&& (Pte.PteHardware & MM_I386_DPTE_COMMITTED);
}
...
{
	Pte.PteHardware &= ~MM_I386_PTE_ADDRESSMASK;
...
{
	Pte.PteHardware &= ~(MM_I386_PTE_READWRITE
	...
	if (PageBits & MM_PROT_WRITE)         Pte.PteHardware |= MM_I386_PTE_READWRITE;
	if (PageBits & MM_PROT_USER)          Pte.PteHardware |= MM_I386_PTE_USERACCESS;
	if (PageBits & MM_MISC_DISABLE_CACHE) Pte.PteHardware |= MM_I386_PTE_CDISABLE;
	if (PageBits & MM_MISC_ACCESSED)      Pte.PteHardware |= MM_I386_PTE_ACCESSED; // do we even need this?
	if (PageBits & MM_MISC_DIRTY)         Pte.PteHardware |= MM_I386_PTE_DIRTY;    // do we even need this?
	if (PageBits & MM_MISC_IS_FROM_PMM)   Pte.PteHardware |= MM_I386_PTE_ISFROMPMM;
	return Pte;
}
...
MMPTE MmBuildPte(MMPFN Pfn, uintptr_t PageBits)
{
	...
	Pte.PteHardware |= MM_I386_PTE_PRESENT;
...
MMPTE MmBuildWasPresentPte(MMPTE OldPte)
{
	...
	OldPte.PteHardware |= MM_I386_DPTE_WASPRESENT;
	return OldPte;
}
...
MMPTE MmBuildPoolHeaderPte(uintptr_t Address)
{
	...
	Address |= MM_I386_DPTE_ISPOOLHDR;
	MMPTE Pte;
...
{
	return (Pte.PteHardware & MM_I386_PTE_ADDRESSMASK) >> 12;
}
...
uintptr_t MmGetPageBitsPte(MMPTE Pte)
{
	...
	if (Pte.PteHardware & MM_I386_PTE_READWRITE)  PageBits |= MM_PROT_WRITE;
	if (Pte.PteHardware & MM_I386_PTE_USERACCESS) PageBits |= MM_PROT_USER;
	if (Pte.PteHardware & MM_I386_PTE_ACCESSED)   PageBits |= MM_MISC_ACCESSED;
	if (Pte.PteHardware & MM_I386_PTE_DIRTY)      PageBits |= MM_MISC_DIRTY;
	if (Pte.PteHardware & MM_I386_PTE_CDISABLE)   PageBits |= MM_MISC_DISABLE_CACHE;
	if (Pte.PteHardware & MM_I386_PTE_ISFROMPMM)  PageBits |= MM_MISC_IS_FROM_PMM;
...
bool MmIsUnsupportedHigherLevelPte(MMPTE Pte)
{
	...
	if (Pte.PteHardware & MM_I386_PTE_PAGESIZE)
		return true;
...
{
	return (MmIsPresentPte(Pte) || MmWasPresentPte(Pte)) && (Pte.PteHardware & MM_I386_PTE_ISFROMPMM);
}
