...
{
	return MmHardwarePte(Pte) & MM_I386_PTE_PRESENT;
}

bool MmIsPoolHeaderPte(MMPTE Pte)
{
	return !MmIsPresentPte(Pte) && (MmHardwarePte(Pte) & MM_I386_DPTE_ISPOOLHDR);
}

bool MmWasPresentPte(MMPTE Pte)
{
	return !MmIsPresentPte(Pte)
		&& !(MmHardwarePte(Pte) & MM_I386_DPTE_ISPOOLHDR)
		&& (MmHardwarePte(Pte) & MM_I386_DPTE_WASPRESENT);
}
...
bool MmIsCommittedPte(MMPTE Pte)
{
	return !MmIsPresentPte(Pte)
		&& !(MmHardwarePte(Pte) & MM_I386_DPTE_ISPOOLHDR)
		&& (MmHardwarePte(Pte) & MM_I386_DPTE_DECOMMITTED);
}
...
{
	MmHardwarePte(Pte) &= ~MM_I386_PTE_ADDRESSMASK;
...
MMPTE MmBuildPte(MMPFN Pfn, uintptr_t PageBits)
{
	...
	MmHardwarePte(Pte) |= MM_I386_PTE_PRESENT;
...
MMPTE MmBuildWasPresentPte(MMPTE OldPte)
{
	...
	MmHardwarePte(OldPte) |= MM_I386_DPTE_WASPRESENT;
	return OldPte;
}
...
{
	return (MmHardwarePte(Pte) & MM_I386_PTE_ADDRESSMASK) >> 12;
}
...
uintptr_t MmGetPageBitsPte(MMPTE Pte)
{
	...
	if (MmHardwarePte(Pte) & MM_I386_PTE_READWRITE)  PageBits |= MM_PROT_WRITE;
	if (MmHardwarePte(Pte) & MM_I386_PTE_USERACCESS) PageBits |= MM_PROT_USER;
	if (MmHardwarePte(Pte) & MM_I386_PTE_ACCESSED)   PageBits |= MM_MISC_ACCESSED;
	if (MmHardwarePte(Pte) & MM_I386_PTE_DIRTY)      PageBits |= MM_MISC_DIRTY;
	if (MmHardwarePte(Pte) & MM_I386_PTE_CDISABLE)   PageBits |= MM_MISC_DISABLE_CACHE;
	if (MmHardwarePte(Pte) & MM_I386_PTE_ISFROMPMM)  PageBits |= MM_MISC_IS_FROM_PMM;
...
bool MmIsUnsupportedHigherLevelPte(MMPTE Pte)
{
	...
	if (MmHardwarePte(Pte) & MM_I386_PTE_PAGESIZE)
		return true;
