...
#define MM_AMD64_PTE_PRESENT    (1ULL <<  0)
#define MM_AMD64_PTE_READWRITE  (1ULL <<  1)
#define MM_AMD64_PTE_USERACCESS (1ULL <<  2)
#define MM_AMD64_PTE_WRITETHRU  (1ULL <<  3)
#define MM_AMD64_PTE_CDISABLE   (1ULL <<  4)
#define MM_AMD64_PTE_ACCESSED   (1ULL <<  5)
#define MM_AMD64_PTE_DIRTY      (1ULL <<  6)
#define MM_AMD64_PTE_PAT        (1ULL <<  7)
#define MM_AMD64_PTE_PAGESIZE   (1ULL <<  7) // in terms of PML3/PML2 entries, for 1GB/2MB pages respectively. Not Used by the kernel
#define MM_AMD64_PTE_GLOBAL     (1ULL <<  8) // doesn't invalidate the pages from the TLB when CR3 is changed
#define MM_AMD64_PTE_ISFROMPMM  (1ULL <<  9) // if the allocated memory is managed by the PFN database
//#define MM_AMD64_PTE_COW        (1ULL << 10) // if this page is to be copied after a write -- TODO: We are supposed to be phasing this one out.
//#define MM_AMD64_PTE_TRANSITION (1ULL << 11) // if this page is in transition (3) (UNUSED)
#define MM_AMD64_PTE_NOEXEC     (1ULL << 63) // aka eXecute Disable
#define MM_AMD64_PTE_PKMASK     (15ULL<< 59) // protection key mask. We will not use it.

#define MM_AMD64_PTE_ADDRESSMASK (0x000FFFFFFFFFF000) // description of the other bits that aren't 1 in the mask:

#define MM_AMD64_DPTE_ISPOOLHDR    (1ULL << 58) // if the PTE actually contains the address of a pool entry (subtracted MM_KERNEL_SPACE_BASE from it)
#define MM_AMD64_DPTE_DECOMMITTED  (1ULL << 3)
#define MM_AMD64_DPTE_COMMITTED    (1ULL << 8)
#define MM_AMD64_DPTE_WASPRESENT   (1ULL << 62)
//#define MM_AMD64_DPTE_BACKEDBYFILE (1ULL << 9)
//#define MM_AMD64_DPTE_SWAPPED      (1ULL << 10)
